...
class B2S_Network_Item {
    ...
    private function getPortaleHtml($networkId, $networkName, $mandantId, $mandantenData, $networkData, $maxNetworkAccount = false, $showAllAuths = false, $addonText = false) {
        ...
        if (in_array($networkId, $this->allowProfil)) {
            ...
            } else if (($networkId == 25 || $networkId == 26 || $networkId == 27 || $networkId == 39 || $networkId == 44) && B2S_PLUGIN_USER_VERSION < 1) {
    ...
    public function getEditTemplateFormContent($networkId, $networkType, $schema) {
        ...
        if ($networkId != 44 && $networkId != 43 && $networkId != 12 && $networkId != 2 && $networkId != 3 && $networkId != 1) {
            if ($networkId == 11) {
        ...
        $content .= '</div>';
        $content .= '</div>';
        if ($networkId == 1 || $networkId == 2 || $networkId == 3 || $networkId == 43 || $networkId == 44) {
    ...
    private function networkPreview($networkId, $networkType, $schema) {
        ...
        switch ($networkId) {
            ...
            case '44':
                ...
                $preview .= '<div class="row">';
                $preview .= '<div class="col-sm-2">';
                $preview .= '<img class="b2s-edit-template-preview-profile-img-44" src="' . esc_url(plugins_url('/assets/images/b2s@64.png', B2S_PLUGIN_FILE)) . '">';
                $preview .= '</div>';
